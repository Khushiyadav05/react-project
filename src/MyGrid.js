...
// Only new theme CSS (remove legacy ag-grid.css)
import 'ag-grid-community/styles/ag-theme-alpine.css';

// Register all community modules
ModuleRegistry.registerModules([AllCommunityModule]);

const MyGrid = ({ gridRef }) => {
  const rowData = [
  ...
  const columnDefs = [
    ...
    { headerName: "Performance", field: "performanceRating", sortable: true, filter: true },
    ...
    { headerName: "Skills", field: "skills", sortable: true, filter: true, valueFormatter: params => params.value.join(", ") }
  ];
  ...
  return (
    <div className="ag-theme-alpine" style={{ height: 700, width: '100%' }}>
      <AgGridReact
        ...
        paginationPageSizeSelector={false} // disables warning
      />
    </div>
  );
};
